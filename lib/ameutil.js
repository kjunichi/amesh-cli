...
const AMESH_BASE_URL = "https://tokyo-ame.jwa.or.jp"
  // 地形図
...
function getImages(idx, urls, cb) {
  if (idx < 0) {
    cb(bufary)
    return
  }
...
exports.getImage = function(cb) {
  ...
  function display(imgdata) {
    ...
    img.onload = () => {
	    //console.log(`img.onload: begin`)
      if(!canvas) {
	      //console.log(`debug: new Canvas`)
  ...
  canvas=null
  getImages(urls.length - 1, urls, (imgs) => {
    display(imgs[0])
    display(imgs[2])
    display(imgs[1])
    cb(canvas.toBuffer())
